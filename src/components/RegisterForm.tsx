...
export default function RegisterForm({ onSwitchLogin, onNeedsVerify }: RegisterFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <button type="button" className={styles.googleBtnDisabled} disabled>
        <svg className={styles.googleIcon} viewBox="0 0 24 24" width="20" height="20">
          <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#ccc"/>
        ...
        {t.register.googleBtn} — Coming soon
      </button>
      ...
      <div className={styles.field}>
        ...
        <input type="text" className={`${styles.input} ${form.username.length >= 3 && usernameAvailable === false ? styles.inputError : ""} ${form.username.length >= 3 && usernameAvailable === true ? styles.inputOk : ""}`}
          placeholder={t.register.usernamePlaceholder} value={form.username}
        ...
        {!checkingUsername && form.username.length >= 3 && usernameAvailable === false && <p className={styles.errorText}>{t.register.usernameError}</p>}
        {!checkingUsername && form.username.length >= 3 && usernameAvailable === true && <p className={styles.okText}>✓ Available</p>}
      </div>
